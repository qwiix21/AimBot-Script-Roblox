...
local aimMode = "Hold" -- "Hold" or "Toggle"
...
local ActivationKeyButton, MinimizeKeyButton, ControlsLabel
...
local function saveSettings()
    local settings = {
        ...
        ignoredTeams = ignoredTeams, ignoredPlayers = ignoredPlayers,
        ...
        aimMode = aimMode, mouseAiming = mouseAiming
    }
...
local function loadSettings()
    if isfile("aimbot_settings.json") then
        ...
        if success and data then
            ...
            ignoredTeams = data.ignoredTeams or {}
...
local function checkWall(target)
    ...
    local ray = workspace:Raycast(camera.CFrame.Position, (target.Position - camera.CFrame.Position).Unit * 1000)
    
    if ray and ray.Instance then
        return not ray.Instance:IsDescendantOf(target.Parent)
    end
...
local function getClosest(cframe)
    ...
    for i,v in pairs(Players:GetPlayers()) do
        if v.Character and v.Character:FindFirstChild(targetPart) and v.Character:FindFirstChild("Humanoid") and v.Character:FindFirstChild("HumanoidRootPart") and v ~= localPlayer then
            ...
            if ignoredTeams[v.Team] then continue end
...
local function checkTrigger()
    ...
    if target and target.Parent:FindFirstChild("Humanoid") then
        ...
        if player and player ~= localPlayer then
            if not teamCheck or not localPlayer.Team or not player.Team or localPlayer.Team ~= player.Team then
                if not ignoredPlayers[player.Name] and not ignoredTeams[player.Team] then
                    autoFire()
                end
            end
        end
        ...
        loop = RunService.RenderStepped:Connect(function()
            ...
            if lockMode and currentTarget and currentTarget.Character and currentTarget.Character:FindFirstChild(targetPart) then
                if currentTarget.Character.Humanoid.Health > 0 then
                    ...
                    if (ssHeadPoint - zz).Magnitude < fov * 2 then
                        local aimSpeed = instantLock and 1 or (rageMode and 0.8 or smoothing / 100)
                        
                        if mouseAiming then
                            local delta = ssHeadPoint - zz
                            local moveDelta = delta / (smoothing * 2)
                        ...
                        if autoShoot and (ssHeadPoint - zz).Magnitude < 30 then
                            autoFire()
                        end
                        return
                    else
...
for _, team in pairs(Teams:GetTeams()) do
    Tabs.Exclusions:AddToggle("Team_" .. team.Name, {
        Title = "Ignore " .. team.Name,
        Description = "Don't aim at this team",
        Default = ignoredTeams[team.Name] or false
    }):OnChanged(function(value)
        ignoredTeams[team.Name] = value
        saveSettings()
    end)
end
...
for _, player in pairs(Players:GetPlayers()) do
    if player ~= localPlayer then
        Tabs.Exclusions:AddToggle("Player_" .. player.Name, {
            Title = "Ignore " .. player.Name,
            Description = "Don't aim at this player",
            Default = ignoredPlayers[player.Name] or false
        }):OnChanged(function(value)
            ignoredPlayers[player.Name] = value
            saveSettings()
        end)
    end
end
...
ActivationKeyButton = Tabs.Settings:AddButton({
    ...
    Callback = function()
        ...
        Fluent:Notify({
            Title = "Key Setup",
            Content = "Press any key to set as activation key",
            Duration = 5
        })
        ...
        keyBindConnection = UserInputService.InputBegan:Connect(function(input, gameProcessed)
            if not gameProcessed and input.UserInputType == Enum.UserInputType.Keyboard then
                ...
                Fluent:Notify({
                    Title = "Key Changed",
                    Content = "New key: " .. getKeyName(activationKey),
                    Duration = 3
                })
            end
        end)
...
MinimizeKeyButton = Tabs.Settings:AddButton({
    ...
    Callback = function()
        ...
        Fluent:Notify({
            Title = "Key Setup",
            Content = "Press any key to set as minimize key",
            Duration = 5
        })
        ...
        keyBindConnection = UserInputService.InputBegan:Connect(function(input, gameProcessed)
            if not gameProcessed and input.UserInputType == Enum.UserInputType.Keyboard then
                ...
                Fluent:Notify({
                    Title = "Key Changed",
                    Content = "New minimize key: " .. getKeyName(minimizeKey),
                    Duration = 3
                })
            end
        end)
...
UserInputService.InputEnded:Connect(function(input, gameProcessed)
    ...
    if input.KeyCode == activationKey and aimMode == "Hold" then
        isActive = false
        toggleAimbot(false)
    end
end)
...
print("🔥 Elite Aimbot v0.7 loaded!")
